...
filename="config_PPSX00.ini" #TO CHANGE
...
generate_config_and_run() {
    ...
    out_filename="Results_PPSX00/step/config_modif_cp_${indiceX}_${indiceY}.ini"
    
    # Use sed to replace the lines in the original file and save to the output file
    sed -e "s/Result dir        = Results\/zzz/Result dir        = .\/Results_PPSX00\/step\/data_${indiceX}_${indiceY}/g" "$filename" > "$out_filename"
