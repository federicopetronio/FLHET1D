...
B0       = Bmax*np.exp(-((x_center - L0)/LB1)**2.)              # Magnetic field within the thruster
B0       = np.where(x_center < L0, B0, Bmax*np.exp(-((x_center - L0)/LB2)**2.)) # Magnetic field outside the thruster
...
def SetInlet(P_In, U_ghost, P_ghost, moment=1):
...
if TIMESCHEME == 'Forward Euler':
    ...
    while time < TIMEFINAL:
        ...
        if (iter %SAVERATE) ==0:
            ...
            i_save += 1
            print("Iter = ", iter,"\tTime = {:.2f}~µs".format(time/1e-6),"\tJ = {:.4f}~A".format(J))
            
        # Set the boundaries
        SetInlet(P[:, 0], U_Inlet, P_Inlet, 1)
...
if TIMESCHEME == 'TVDRK3':
    ...
    while time < TIMEFINAL:
        ...
        if (iter %SAVERATE) == 0:
            SaveResults(P, U, P_Inlet, P_Outlet, J, V, x_center, time, i_save)
            i_save += 1
            print("Iter = {}".format(iter),"\t Time = {:.2f} µs".format(time/1e-6),"\t J = {:.4f} A".format(J),"\t V = {:.2f} V".format(V))
        #################################################
        #           FIRST STEP RK3
        #################################################
        ...
        #################################################
        # Set the boundaries
        SetInlet(P[:, 0], U_Inlet, P_Inlet, 2)
        SetOutlet(P[:, -1], U_Outlet, P_Outlet)
        ...
        #################################################
        # Set the boundaries
        SetInlet(P[:, 0], U_Inlet, P_Inlet, 3)
        SetOutlet(P[:, -1], U_Outlet, P_Outlet)
