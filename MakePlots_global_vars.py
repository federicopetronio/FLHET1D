...
nu_anom_eff     = alpha_B * (phy_const.e/phy_const.m_e) * Barr
...
def compute_Rei_saturated(fP):
    ...
    vi = fP[2,:]
    Te = fP[3,:]
    uB = np.sqrt(phy_const.e*Te / Mi) # Bohm velocity or sound speed.

    deriv_factor = gradient(vi*ni*Te, x_center)
...
@njit
def compute_phi(fE, fJ, fV):
    
    phi = fV - fJ * Rext - cumTrapz(fE, Delta_x)  # Discharge electrostatic potential
    return phi


@njit
def compute_heat_flux(fP_ext):
    #############################################################
    #       We give a name to the vars to make it more readable
    #############################################################
    ...
    if wall_inter_type == "Default":
        ...
        nu_iw = (4./3.)*(1./(R2 - R1))*np.sqrt(phy_const.e*Te/Mi)
        # Limit the wall interactions to the inner channel
        nu_iw[x_center_extended> LTHR] = 0.0
        nu_ew = nu_iw / (1.0 - sigma)  # Electron - wall collision rate        
    ...
    nu_m = (
        ng * KEL + alpha_B_extended * wce + nu_ew
    )
    ...
    grad_Te  = (Te[1:] - Te[:-1]) / ( x_center_extended[1:] - x_center_extended[:-1] )
...
for i_save, file in enumerate(files):
    ...
    phi = compute_phi(Efield, J, V)
    
    Rei_emp = compute_Rei_empirical(P)
    #print(Rei_emp.shape)

    Rei_sat = compute_Rei_saturated(P)
    #print(Rei_sat.shape)

    #print(f"{i_save}: P_LC", P_LeftGhost.shape)
    #print("P_RC", P_LeftGhost.shape)
    #print("P", P.shape)

    q_array = compute_heat_flux(np.concatenate([P_LeftGhost, P, P_RightGhost], axis=1))
    ...
    ax_b.plot(x_center*100, Barr*1000, 'r:')
