...
generate_config_and_run() {
    ...
    alphaB1=$(awk "BEGIN { printf \"%.6f\", 0.01 + $indiceX / 1000 }")  
    alphaB2=$(awk "BEGIN { printf \"%.6f\", 0.01 + $indiceY / 1000 }") 
...
for indiceX in {-5..5}; do
    for indiceY in {-5..5}; do
        ...
        while (( $(jobs | wc -l) >= 11 )); do # We choose 11 max as we make 121 simulations in this configuration
            sleep 1
        done
    done
done
