...
if __name__ == '__main__':
    ...
    for i_save, file in enumerate(files):
        ...
        ax_b = ax[7].twinx()
        ax[7].plot(x_center*100, P[4,:]/1000, label = "$v_{ex}$")
        ax[7].set_ylabel('$v_e$ [km/s]', fontsize=axisfontsize)
        ax_b.plot(x_center*100, P[5,:], 'g--', label = "$v_{ey}$")
        ...
        ax[7].legend(fontsize=tickfontsize, loc='upper right')
        ax_b.legend(fontsize=tickfontsize, loc='lower right')
